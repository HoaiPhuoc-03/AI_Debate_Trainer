...
class SupabaseStore:
    ...
    def ensure_profile(
        ...
        **profile_fields,
        ...
        if existing:
            has_changes = False
            if email is not None and existing.get("email") != email:
                has_changes = True
            if display_name is not None and existing.get("display_name") != display_name:
                has_changes = True
                ...
                if k == "metadata":
                    ...
                    if isinstance(v, dict):
                        for mk, mv in v.items():
                            if existing_meta.get(mk) != mv:
                                has_changes = True
                                break
                elif v is not None and existing.get(k) != v:
                    has_changes = True
                    break
            if not has_changes:
                return existing
